export function setUbication() {
  ...
  if (loggedUserInfo) { // Si la información del usuario se encuentra de manera local
  ...
  if (user && user.tipo == 'Contratista' && pathActual == '/estudiante') {
    location.assign('/contratista')
  }
  if (user && user.tipo == 'Estudiante' && pathActual == '/contratista') {
