...
const BedRoom = (props) => {
  ...
  const handleDeleteHabitacion = (event) => {
    ...
    const habitacionUrl = 'https://crz-backend-ti-emergentes.azurewebsites.net/v1/sextob/api/habitacion'
  ...
  const handleAlquilarHabitacion = (event) => {
    ...
    const idHabitacion = event.target.attributes.itemid.nodeValue;
    const estudianteUrl = 'https://crz-backend-ti-emergentes.azurewebsites.net/v1/sextob/api/estudiante'
    const habitacionUrl = 'https://crz-backend-ti-emergentes.azurewebsites.net/v1/sextob/api/habitacion'
    ...
    <Col sm={smx} md={mdx} lg={lgx} className='mb-4'>
      <Card>
        ...
        <Card.ImgOverlay className="d-flex flex-column justify-content-between">
          ...
          </div>
          {
            ...
            user && user.tipo == 'Estudiante' && pathActual == '/'
              ? <Button
                  size='sm'
                  itemID={id}
                  className='w-50'
              ...
              : user && user.tipo == 'Estudiante' && pathActual == '/estudiante'
                  ? <Button
                      size='sm'
                      itemID={id}
                      className='w-75'
      ...
      <Card.Footer className='border-0 text-break'>
        ...
        </div>
        {
          ...
          */
            user && user.tipo == 'Contratista' && pathActual == '/contratista' && rent == false
            ? <div>
                <Button
            ...
            : user && user.tipo == 'Contratista' && pathActual == '/contratista' && rent == true
